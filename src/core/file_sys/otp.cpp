...
// A placeholder OTP used when no real console-dumped OTP is available, so that titles requiring
// online LLE modules (e.g. eShop, System Settings) can still boot when that hack is enabled.
...
Loader::ResultStatus OTP::Load(const std::string& file_path, std::span<const u8> key,
    ...
    if (!file.IsOpen()) {
        if (Settings::values.enable_required_online_lle_modules.GetValue()) {
            std::memcpy(&temp_otp, dummy_otp, sizeof(dummy_otp));
        } else {
    ...
    if (temp_otp.hash != digest) {
        // The dummy OTP above intentionally doesn't hash-verify; only log instead of failing so
        // the online-LLE-modules fallback can still proceed.
